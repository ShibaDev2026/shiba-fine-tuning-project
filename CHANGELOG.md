...
- **RAG/Agentic 論文庫擴充：`docs/references/papers/` 05–26（22 篇全文分析，2026-06-22）** — harness 自主迴圈：subagent 並行搜尋（Google Scholar/arXiv，賽道 A=Agentic/自適應檢索策略、賽道 B=召回品質+模式重用）→ triage 23 stub 入 `pending/` → 逐篇全文分析成 `NN_名稱/paper.md`（對齊既有 01–04 格式：對應 Layer/核心結論帶數字/方法拆解/速查接地/侷限）+ 更新 `PAPERS_INDEX.md`。
  - **流程 gate（advisor 校準）**：分析前批量 WebFetch 驗證 6 篇後截止日 arxiv ID（全真實存在、無幻影論文）；數字經 WebFetch 摘要抽取、未逐一對 PDF 核（已於 index 加 provenance 標註，驅動花費前須回核原文）。
  ...
  - 05–07 附 PDF；08–26 因 arxiv 限速暫缺 PDF（paper.md 全文分析不受影響、可日後補抓）。`pending/` 已清空。⚠ zsh 不對未加引號變數 word-split，過程曾誤建 5 個空殼目錄、已清理。
...
- **Layer 1 召回日誌：系統 prompt 污染 + 孤兒 pending 洩漏（2026-06-21）** — 稽核今日 `recall_logs/20260621.txt` 揪出兩個對症問題（根因同一個：UserPromptSubmit hook 對「所有」Claude Code 觸發都跑召回，含記憶系統/harness spawn 的內部 LLM 呼叫）：
  - **Part 1（根因/預防）查詢側結構性 gate `rag.py:is_system_meta_query()`** — 攔下 harness/remember 外掛產生的系統 prompt（`You are summarizing a Claude Code session` daily 摘要 / `Apply maximum non-destructive compression` 壓縮 / `<task-notification>` 子任務通知 / `This session is being continued` context 續接）：純前綴比對、零 DB、放 hook 最前，命中即不召回/不寫 log/不通知/不寫 pending。**用 content-prefix 而非 env marker**：來源是不可控第三方外掛/harness，無法注入環境標記；錨定「穩定且夠長」的開頭前綴（固定 prompt 檔/結構標籤、不逐對話漂移），避免誤殺 Shiba 引用這些字串的真實討論。與 `is_low_signal_query`（資料驅動同意詞）正交。
  - **Part 2（洩漏自癒）`recall_log.py:clear_pending()` + Stop hook 一律清 pending** — 根因：`_append_recall_answer` 在 `has_pending` 為真後有四條早退路徑（transcript 找不到 / parse 失敗 / 無 assistant 回答 / scrub 不可用），**全都不清 pending** → 真實 session 也會洩漏孤兒（不只系統 session）。修：`has_pending` 為真後把補回答全程包進 `try/finally`，`finally` 一律 `clear_pending`（session 結束＝配對最後機會、無答案的 pending 即死標）。採 deterministic clear 而非時間 prune（advisor：prune 治標、clear 治本）。
  - **驗收**：新測 3 passed（`is_system_meta_query` 系統 prompt True×4 / 真實查詢 False×3；`clear_pending` 清孤兒+重複清安全）；`tests/memory/` 48 passed（3 pre-existing `test_db` fail 與本次無關）；真實 hook smoke：系統 prompt→`{}`+0 新 pending+不寫日檔；真實查詢→照常寫 pending（gate 不誤殺）。⚠ 殘留：已 ingest 進 `exchange_embeddings` 的系統 prompt 雜訊仍會被真實查詢召回（屬另一筆 ingestion 清洗債，本次不處理）。
