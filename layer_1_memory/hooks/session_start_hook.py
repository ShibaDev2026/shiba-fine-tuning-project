...
輸出格式（符合 Claude Code Hook 規範）：
{
  "hookSpecificOutput": {
    "memoryContext": "## 相關歷史記憶（top 3）\n..."
  }
}

若無相關記憶或 DB 尚未初始化，輸出空的 hookSpecificOutput。
...
def main() -> None:
    ...
    # 預設輸出（無記憶時的空回應）
    empty_output = json.dumps({"hookSpecificOutput": {}}, ensure_ascii=False)
    ...
    try:
        ...
        if memory_context:
            ...
            logger.debug("RAG 無相關記憶，輸出空 context")
